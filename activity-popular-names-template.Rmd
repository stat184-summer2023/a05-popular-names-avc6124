...
head(BabyNames) #summary function 1 
view(BabyNames) #summary function 2
?BabyNames
```

Question 1: What are the variables are available?
  -The variables available are Name, Sex, Count, Year
  
Question 2: What is a case?
  -A case in this data set is a name, sex, and year 
...
The "Sex" variable of the baby does not appear on the graph.
...
The "Count" variable has been transformed as "Popularity." Also, the sex variable has been grouped so the names are represented as one variable and not split by gender.

### Step 4

##### Have cases been filtered out?

All cases that don't include the names "Abraham," "Franklin," or "Woodrow" have been filtered out.


##### Have cases been grouped and summarized within groups in any way?

Te names with two genders have been grouped so any cases of multiple genders concering the names "Abraham," "Franklin," or "Woodrow" have been grouped


##### Have any new variables been introduced?  If so, what the relationship between the new variables and existing variables?

A new name for the count variable have been introduced as "popularity" the data inside the variables are the same just the name has changed.
...
picked_names <- 
  c("Alice", "Robert", "Aaron", "Julie", "Adam")

picked_names <-
  BabyNames %>%
  filter(name %in% picked_names)
...
ggplot(total_births) +
  aes(x = year, y = total_births, colour = name) +
  geom_line() +
